...
LOG_FILE="/var/log/post-install-arch-logs.log" # Corrigido o caminho do log
...
# Removida a linha: ln -s /var/log /var/logs (não é necessária e pode causar erro)
...
chown "$USER":"$USER" "/home/$USER/TEMP" # Adicionado: Define o dono para o usuário
...
chown "$USER":"$USER" "/home/$USER/Documentos/Planilhas" # Adicionado: Define o dono para o usuário
...
chown "$USER":"$USER" "/home/$USER/AppImages/" # Adicionado: Define o dono para o usuário
...
pacman -S --noconfirm yay # Assuming yay is in official repos or chaotic-aur
check_status "pacman -S yay"

# Instala Chrome (via yay, pois geralmente é um pacote AUR)
log_message "Instalando Google Chrome (via yay)..." # Correção do typo e ajuste para yay
sudo -u "$USER" yay -S --noconfirm google-chrome # Executa yay como o usuário normal
...
pacman -U 'https://github.com/pedrodev2025/script-de-post-install-pro-arch-linux/raw/refs/heads/main/cdn-alpaca/python-mammoth-1.9.1-4-any.pkg.tar.zst' --noconfirm
check_status "pacman -U python-mammoth.pkg.tar.zst"
...
# Instala Gnome Software e plugin Flatpak
log_message "Instalando Gnome Software e plugin Flatpak..."
pacman -S --needed --noconfirm gnome-software gnome-software-plugin-flatpak # Adicionado o plugin
